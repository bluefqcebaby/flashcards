...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (nextOpen: boolean | ((currentOpen: boolean) => boolean)) => {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: We persist the sidebar state with the same lightweight cookie strategy as the upstream shadcn component.
...
function SidebarGroupLabel({
  ...
  return useRender({
    ...
    props: mergeProps<"div">(
      {
        className: cn(
          "flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium tracking-[0.2em] text-sidebar-foreground/60 uppercase ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-200 ease-linear group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
          className
        ),
      },
      props
